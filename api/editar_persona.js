    ...
    try {
        ...
        if (fecha_caducidad) {
            ...
            if (fechaCaducidadDate <= new Date()) {
                throw new Error('La fecha de caducidad debe ser una fecha futura');
            }
        }
        ...
        if (updateFoto) {
            sql = `UPDATE personas SET 
               ...
               notas = $9, fecha_caducidad_qr = $10
               WHERE matricula = $11`;
            params = [matricula, nombres, apellidos, curp, tipopersona, idCarrera, estado, fotoPerfil, notas, fecha_caducidad, matriculaOriginal];
        } else {
            sql = `UPDATE personas SET 
               matricula = $1, nombres = $2, apellidos = $3, curp = $4, 
               tipo_persona = $5, id_carrera = $6, estado = $7,
               notas = $8, fecha_caducidad_qr = $9
               WHERE matricula = $10`;
            params = [matricula, nombres, apellidos, curp, tipopersona, idCarrera, estado, notas, fecha_caducidad, matriculaOriginal];
        }
